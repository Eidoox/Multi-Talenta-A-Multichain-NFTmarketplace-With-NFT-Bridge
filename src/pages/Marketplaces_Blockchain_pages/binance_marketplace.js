...
const Binance_Marketplace = ({accounts ,chainid,createnftscontractbinance,nftmarketplacebinancecontract}) => {
    const iswalletconnected = Boolean(accounts[0]);
    const [marketitems , setmarketitems] = useState([]);
    const toast = useToast();
    console.log(marketitems);
    
    const explore_not_sold_nfts_binance = async () => {
        const nftsitems = await nftmarketplacebinancecontract.getnotsoldnfts();
        const nftscount = nftsitems.length;
        let nftitemsnotsold = [];

        for (let i = 0 ; i< nftscount ; i++){
            if (nftsitems[i].issold == 0 ){
                const metadataurl = await createnftscontractbinance.tokenURI(nftsitems[i].tokenid);
                ...
                const totalprice_marketitem = await nftmarketplacebinancecontract.totalpricewith_marketfees(nftsitems[i].itemid);
                const priceofnftindollar = await nftmarketplacebinancecontract.getLatestPriceOfMaticVsUSD();
    ...
    const buynftfrombinancemarket = async (marketitem) => {

        try {
            
        const buytx = await (await nftmarketplacebinancecontract.buynft(marketitem.itemid, { value: marketitem.price })).wait();
    ...
    useEffect(() => {
        ...
        if (iswalletconnected == false) {
            toast({
                ...
                description: "You have to Connect your MetaMask Wallet to view Binance Marketplace NFT items",
                status: 'info',
                duration: 2100,
                isClosable: true,
                position: 'up',   
              ...
              <Box bgGradient='linear(to-r, azure,white,azure,white)' height="1200px">
                    ...
                    { iswalletconnected ? (
                                                    ...
                                                    <Card key={idx}   >
                                                                ...
                                                                <Box height="50px">
                                                                    ...
                                                                    <Box height="30px">
                                                                        <Text fontSize={18} ml="15px" fontWeight="bold">
                                                                            <a href={`https://testnet.bscscan.com/address/${nft.seller}`}> 
                                                                    ...
                                                                    <Box height="35px">
                                                                        <Text fontSize={18} ml="15px" fontWeight="bold">
                                                                            Buy for: {ethers.utils.formatEther(nft.price)} BNB
                                                                        </Text>
                                                        ...
                                                        <Card.Footer>
                                                                 ...
                                                                 <Button 
                                                                    ...
                                                                    height="60px"
                                                                    ml={12}
                                                                    onClick= {() => buynftfrombinancemarket(nft)}
                        ...
                        <Flex  justify="center" fontWeight='extrabold' fontSize="37px" bgClip='text'  bgGradient='linear(to-r, blue.900, red.500)' padding={50} >
                            Connect Wallet to view NFT item on Binance Martketplace
                        </Flex>
            ...
            >   
                <Box bgGradient='linear(to-r, azure,white,azure,white)' height="1080px">
                    <Flex justify="center"  fontWeight='extrabold' fontSize="43px"  bgClip='text'  bgGradient='linear(to-r, cyan.900, pink.800,cyan.800,)'  padding={50} >
                                   Switch Your Network to Binance Test Network to view NFT items.
                    </Flex>
...
export default Binance_Marketplace;