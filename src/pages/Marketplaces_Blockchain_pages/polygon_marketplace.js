...
const Polygon_Marketplace = ({accounts ,chainid,createnftscontractpolygon,nftmarketplacepolygoncontract}) => {
    const iswalletconnected = Boolean(accounts[0]);
    const [marketitems , setmarketitems] = useState([]);
    const toast = useToast();
    console.log(marketitems);
    
    const explore_not_sold_nfts_polygon = async () => {
        const nftsitems = await nftmarketplacepolygoncontract.getnotsoldnfts();
        const nftscount = nftsitems.length;
        let nftitemsnotsold = [];

        for (let i = 0 ; i< nftscount ; i++){
            if (nftsitems[i].issold == 0 ){
                const metadataurl = await createnftscontractpolygon.tokenURI(nftsitems[i].tokenid);
                ...
                const totalprice_marketitem = await nftmarketplacepolygoncontract.totalpricewith_marketfees(nftsitems[i].itemid);
                const priceofnftindollar = await nftmarketplacepolygoncontract.getLatestPriceOfMaticVsUSD();
    ...
    const buynftfrompolygonmarket = async (marketitem) => {

        try {
            
        const buytx = await (await nftmarketplacepolygoncontract.buynft(marketitem.itemid, { value: marketitem.price })).wait();
    ...
    useEffect(() => {
        ...
        if (iswalletconnected == false) {
            toast({
                ...
                description: "You have to Connect your MetaMask Wallet to view Polygon Marketplace NFT items",
                status: 'info',
                duration: 2100,
                isClosable: true,
                position: 'up',   
              ...
              <Box bgGradient='linear(to-r, azure,white,azure,white)' height="1200px">
                    ...
                    { iswalletconnected ? (
                                ...
                                <SimpleGrid columns={6} spacingX='60px' spacingY='20px'>
                                    { marketitems.map((nft,idx) => (
                                        <Col >
                                                ...
                                                <Box  border="1px" borderColor='lightblue' borderRadius="15px" padding={3} pb={1}  width="240px" height="568px">
                                                    <Card key={idx}   >
                                                        <VStack
                                                        divider={<StackDivider borderColor='white' />}
                                                        spacing={1}
                                                                ...
                                                                <Box height="50px">
                                                                    ...
                                                                    <Box height="30px">
                                                                        <Text fontSize={18} ml="15px" fontWeight="bold">
                                                                            <a href={`https://mumbai.polygonscan.com/address/${nft.seller}`}> 
                                                                    ...
                                                                    <Box height="45px">
                                                                        <Text fontSize={18} ml="15px" fontWeight="bold">
                                                                            Buy for: {ethers.utils.formatEther(nft.price)} MATIC
                                                                        </Text>
                                                        ...
                                                        <Card.Footer>
                                                                 ...
                                                                 <Button 
                                                                    ...
                                                                    height="60px"
                                                                    ml={12}
                                                                    onClick= {() => buynftfrompolygonmarket(nft)}
                        ...
                        <Flex  justify="center" fontWeight='extrabold' fontSize="37px" bgClip='text'  bgGradient='linear(to-r, blue.900, red.500)' padding={50} >
                            Connect Wallet to view NFT item on Polygon Martketplace
                        </Flex>
            ...
            >   
                <Box bgGradient='linear(to-r, azure,white,azure,white)' height="1080px">
                    <Flex justify="center"  fontWeight='extrabold' fontSize="43px"  bgClip='text'  bgGradient='linear(to-r, cyan.900, pink.800,cyan.800,)'  padding={50} >
                                   Switch Your Network to Mumbai Polygon Network to view NFT items.
                    </Flex>
...
export default Polygon_Marketplace;